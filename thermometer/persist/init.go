...
func Syncdb() {
	...
	use_database(db, DBname)
	create_table(db, STable)
	...
	//insert_data_stmt(db, stableStmt)
	//select_data_stmt(db, stableStmt)

	fmt.Printf("\n======== end demo test ========\n")
}

func drop_database(db *sql.DB, dbname string) {
	st := time.Now().Nanosecond()
	res, err := db.Exec("drop database " + dbname)
	checkErr(err)
...
func create_database(db *sql.DB, dbname string) {
	st := time.Now().Nanosecond()
	// create database
...
func use_database(db *sql.DB, dbname string) {
	st := time.Now().Nanosecond()
	// use database
	res, err := db.Exec("use " + dbname) // notes: must no quote to db name
	checkErr(err)
...
func create_table(db *sql.DB, stable string) {
	st := time.Now().Nanosecond()
	// create table
	res, err := db.Exec("create table " + stable + " (ts timestamp, degree double) tags(tenant smallint, location binary(20))")
	checkErr(err)
...
func create_table_stmt(db *sql.DB, table string) {
	st := time.Now().Nanosecond()
	// create table
	// (ts timestamp, id int, name binary(8), len tinyint, flag bool, notes binary(8), fv float, dv double)
	res, err := db.Exec("create table " + stable + " (ts timestamp, degree double) tags(tenant smallint, location binary(20))")
	stmt, err := db.Prepare("create table ? using thermometer tags (?, ?)")
	checkErr(err)

	res, err := stmt.Exec(table, "ts", "id", "name", "len", "flag", "notes", "fv", "dv")
	checkErr(err)
